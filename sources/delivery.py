...
def normalize(raw, today=None):
    ...
    # 2) 근시일 도착/수령(예: 교보 "내일 오전 7시 전 도착", 예스24 "내일…(6/12,금) 도착예정",
    #    알라딘 "내일 수령") → 당일 배송.
    #    글자에 날짜가 있어도 그 날짜는 '도착일'이므로 출고예정으로 보지 않는다.
    #    (반대로 '출고/발송' + 날짜 는 예약/입고 → 아래 3) 날짜 규칙으로 넘어감.
    #     "오늘 주문하면 6/17 출고" 는 도착/수령 단어가 없어 여기서 안 걸리고 3)으로 감)
    if any(n in flat for n in _NEAR) and any(a in flat for a in _ARRIVE):
        return "당일 배송"
