...
# 저자 괄호가 '역할어'인지 '본명'인지 구분용
_ROLE_WORDS = ("지은이", "옮긴이", "글그림", "그림글", "원저", "원작", "지음", "옮김",
               "엮음", "편저", "공저", "감수", "해설", "그림", "글", "저", "역", "편", "만화", "작화")
...
def author_core(s):
    ...
    s = re.split(r"[,/·]|\s외\b|\s외$", s)[0].strip()
    # 괄호 처리: 역할어 괄호는 버리고, '본명'으로 보이는 괄호는 그 본명을 사용
    #   예) "채부심(채상욱)"·"채국장(채상욱)" -> "채상욱" (필명 통일),  "김영하(지은이)" -> "김영하"
    m = re.search(r"\(([^)]+)\)", s)
    if m:
        inner = m.group(1).strip()
        if any(r in inner for r in _ROLE_WORDS):
            s = (s[:m.start()] + s[m.end():]).strip()   # 역할 괄호 제거
        else:
            s = inner                                    # 본명 괄호 → 본명 사용
